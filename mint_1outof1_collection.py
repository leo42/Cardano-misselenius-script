...
def extra_meta(knightzee):
    metadata = knightzee
    metadata["name"] = "The Brothership pool - Knightzee " +   str(knightzee["serial"])
    metadata["copyright"] = "Copyright  @2022 jobjeez.com "
...
if __name__ == "__main__":
    ...
    knigtzeez_list=[]
    for knightzee in archive:
        knigtzeez_list.append(knightzee)
        if  len(knigtzeez_list)>=Batch_size:
            send_batch( knigtzeez_list)
            knigtzeez_list=[]
